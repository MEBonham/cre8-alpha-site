...
$.TOC = function(el, scope, options){
	...
	toc.init = function(){
		...
		toc.$el.append("<"+ toc.options.containerType +">"+toc.buildTOC()+"</"+ toc.options.containerType +">");						
	...
	toc.buildTOC = function(){
		...
		if(toc.options.nested) { toc.html = "<"+toc.options.itemType+">\n" + toc.html + "</"+toc.options.itemType+">\n"; }
    ...
    var i;
		if(new_depth > toc.current_depth){
			...
			for(i = toc.current_depth; i < new_depth; i++){
				toc.html += '<' + toc.options.containerType + '>' + "\n <"+toc.options.itemType+">\n";
			}
